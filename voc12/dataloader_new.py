...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        spixel = np.asarray(imageio.imread('./voc_superpixels/' + name_str +'.png'))
        spixel = spixel[..., 0] + spixel[..., 1] * 256 + spixel[..., 2] * 65536 + 1

        if self.resize_long:
            #img = imutils.random_resize_long(img, self.resize_long[0], self.resize_long[1])
            img, spixel = imutils.random_resize_long_sp(img, spixel, self.resize_long[0], self.resize_long[1])

        if self.rescale:
            img = imutils.random_scale(img, scale_range=self.rescale, order=3)
            print('rescale is triggered, please rewrite the code for sp')
        ...
        if self.hor_flip:
            ...
            img, spixel = imutils.random_lr_flip_sp(img, spixel)

        if self.crop_size:
            if self.crop_method == "random":
                #img = imutils.random_crop(img, self.crop_size, 0)
                img, spixel = imutils.random_crop_sp(img, spixel, self.crop_size, 0)
            else:
                img = imutils.top_left_crop(img, self.crop_size, 0)
                print('top_left_crop is triggered, please rewrite the code for sp')
        ...
        spixel = sp_lookup[sp.ravel()].reshape(sp.shape)

        return {'name': name_str, 'img': img, 'spixel': spixel}
