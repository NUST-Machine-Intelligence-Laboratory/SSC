...
def random_resize_long_sp(img, sp, min_long, max_long):
    target_long = random.randint(min_long, max_long)
    h, w = img.shape[:2]
    ...
    return pil_rescale(img, scale, 3), pil_rescale(sp, scale, 0)
...
def random_lr_flip_sp(img, sp):

    if bool(random.getrandbits(1)):
        if isinstance(img, tuple):
            return [np.fliplr(m) for m in img], [np.fliplr(m) for m in sp]
        else:
            return np.fliplr(img), np.fliplr(sp)
    else:
        return img, sp
...
def random_crop_sp(images, sp, cropsize, default_values):

    if isinstance(images, np.ndarray): images = (images,)
    if isinstance(default_values, int): default_values = (default_values,)
    if isinstance(sp, np.ndarray): sp = (sp,)
    ...
    new_images = []
    new_sp = []
    for img, sp, f in zip(images, sp, default_values):
        ...
        cont_sp = np.ones((cropsize, cropsize), sp.dtype)*f
        cont[box[0]:box[1], box[2]:box[3]] = img[box[4]:box[5], box[6]:box[7]]
        cont_sp[box[0]:box[1], box[2]:box[3]] = sp[box[4]:box[5], box[6]:box[7]]
        new_images.append(cont)
        new_sp.append(cont_sp)
