...
from networks.deeplabv2 import Deeplabv2
...
def main_worker(gpu, args: Dict[str, Any]):
    ...
    train_dataset = VOCSegmentationPseuMask(
                                            ...
                                            is_aug=True, 
                                            mask_dir='./data/sem_seg',
                                            pseumask_dir='./data/mask_irn')
    ...
    dataloader_dict, sampler_dict = get_dataloaders(
        config={
			"batch_size": 16, #per GPU, total batch_size is 16
			"num_workers": 1,
			"drop_last": True
		},
    ...
    network1 = Deeplabv2(
        backbone={
			"pretrain": "./data/model_zoo/resnet-101_v2.pth",
			"variety": "resnet-B",
			"depth": 101,
			"out_indices": [4],
			"output_stride": 8,
